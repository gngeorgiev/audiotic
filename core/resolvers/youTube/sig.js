 ...
 */
exports.getTokens = async function (html5playerfile) {
  var key, cachedTokens;
  var rs = /(?:html5)?player-([a-zA-Z0-9\-_]+)(?:\.js|\/)/.exec(html5playerfile);
...
exports.extractActions = function (body) {
  ...
  var myreg = '(?:a=)?' + obj + '\\.(' + [reverseKey, sliceKey, spliceKey, swapKey].join('|') + ')\\(a,(\\d+)\\)';
 ...
 * @param {Boolean} debug
 */
exports.setDownloadURL = function (format, sig) {
  var decodedUrl;
  if (format.url) {
 ...
 * @param {Boolean} debug
 */
exports.decipherFormats = function (formats, tokens) {
